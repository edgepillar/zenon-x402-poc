...
The controller slice is source-only, default-off and local-test-only. `prepareGateBPublicWsInputsForReview` synchronously validates and snapshots one exact initial schema, captures the run name once, constructs one immutable quick-tunnel bootstrap and retains exactly one opaque lease. Its order is exactly `PROVISION_ENDPOINT`, tunnel launch, one fresh readiness check, one `PREPARE`, then `GATE_B_CONTROLLER_REVIEW_REQUIRED_RUN_NOT_AUTHORIZED`. The API deliberately exposes no capability or cancellation surface until those already bounded operations reach review. `authorizeAndPreflightGateBPublicWsInputs` consumes one exact review containing an independently obtained configuration digest and the plaintext-transport, payment and publication acknowledgements, but no run name. It then performs exactly one fresh readiness check, one `AUTHORIZE`, and the filesystem-only exceptional preflight, ending at `GATE_B_CONTROLLER_PREFLIGHT_VALID_RUN_NOT_AUTHORIZED`. Every asynchronous dependency result is trap-safely required to be an unmodified native Promise before awaiting; no hostile/custom thenable is assimilated. Cancellation or duplication during an active reviewed stage disables future actions, waits for that exact stage and exact tunnel closure, and exposes quarantine as its first terminal result. Preflight adds no readiness call, marker, controller artifact, run directory, journal, capture or evidence output. A later run slice must supply separate authority and its own fresh readiness.

The next source-only slice adds the same-process operator coordinator and its fixed human front-end. The direct command is exactly fixed `/usr/bin/env -i`, the absolute current runtime and the audited front-end module with no arguments and an empty environment. A synchronous TTY eligibility check precedes every child. The outer launcher directly starts and captures a fixed detached watchdog and then a fixed detached reaper as siblings. It writes only the captured watchdog PID to the reaper in one exact bounded private target frame with EOF; no process identity is accepted from IPC or exposed in arguments, environment, files, output or status. The reaper installs its exclusive outer-liveness cleanup before payload-free `READY`. Only after the joined target write and `READY` does the outer send the watchdog's dedicated private fieldless `START` and return a frozen setup capability. Before `START` the watchdog reads no bootstrap bytes, opens no operational resource, starts no descendant and emits no operational milestone.

Only after setup does a trusted controlling TTY receive the phase-one prompt and supply the first paste-only canonical JSON line under temporary raw-mode echo suppression. The complete bootstrap arrives within 30 seconds. After that front-end clock completes, submission joins a one-shot fieldless `BOOTSTRAP_OPEN`/`BOOTSTRAP_OPENED` private barrier before canonical bytes reach the watchdog. Only after the fixed review-required status does the front-end accept the three acknowledgements within 120 seconds; after that clock completes, one-shot fieldless `REVIEW_OPEN`/`REVIEW_OPENED` barriers synchronously open the watchdog and inner coordinator readers before canonical review bytes are written. The barriers are bounded transport handshakes and cannot create an earlier user-input deadline. Editing controls and early, surplus, duplicate, stale or phase-confused input or barriers fail closed. Terminal recovery is limited to the captured Node `isRaw` Boolean on normal completion, handled failure, `SIGINT`, `SIGTERM`, process `disconnect`, and raw-mode control-byte rejection or cancellation; uncertainty is terminal quarantine. There is no guarantee for `SIGKILL` or `SIGSTOP`, and no `SIGHUP`, `SIGQUIT`, or `SIGTSTP` handler is claimed outside raw-input control-byte rejection. The PTY test proves no echo and captured Boolean raw-state recovery, not complete termios equivalence. Coordinator, reviewer, controller-only public-input operations, tunnel supervisor and runtime are non-detached and inherit the watchdog group. Both siblings have distinct outer-liveness descriptors that operational descendants do not inherit. No bootstrap or review payload uses arguments, environment, output, temporary files, shell or `PATH` lookup.

The coordinator retains one controller capability and tunnel lease. After the mandatory pause it launches one fixed non-detached reviewer for at most 30 seconds. That independently implemented reviewer opens exactly seven protected leaves and never the wallet; proves authorization absent, distinct stable file identity and the shared ownership/mode/link/ACL rules; validates all cross-file and configuration semantics; brackets final verification with clean-source-revision attestation; and sends only one bounded private digest result. The merged controller independently recomputes the digest during `AUTHORIZE`. Automated validator independence is not evidence of a second human or human comprehension, and the acknowledgements remain operator assertions. The specialized controller entrypoint launches the tunnel supervisor in inherited-group mode, keeping it and its non-detached runtime child in the watchdog group; the ordinary launcher remains detached. That topology seam is not a claim that configuration has no selector: the bootstrap documents and validates both exact `telemetryMode` choices. Inner inherited cleanup never probes or signals a negative group. A valid preflight emits `GATE_B_CONTROLLER_PREFLIGHT_VALID_RUN_NOT_AUTHORIZED` and remains non-authorizing. The coordinator holds until payload-free `STOP` or its nine-minute lifetime and stops and waits for the controller exactly once.

After reaper `READY`, cleanup ownership is serialized: the outer requests reaper cleanup and does not concurrently signal the watchdog group. Payload-free `ABSENT` is valid only after the reaper has proved ESRCH for that captured group. Reaper loss, timeout or false `ABSENT` makes the outer terminate and wait for the captured reaper group first, then probe and if necessary apply TERM, 500-millisecond grace, KILL and ESRCH proof to the captured watchdog group. Clean closure joins watchdog exit, reaper `ABSENT` and exit, and repeated independent absence proofs for both captured groups. The proof covers the exact same-group reviewer, tunnel supervisor and runtime child, not arbitrary descendants. Before `READY`, responsive inert-watchdog outer-death cleanup and live-outer TERM/KILL cleanup of a hung inert watchdog are covered. The accepted dual-fault boundary is outer hard death before `READY` plus an already unresponsive inert watchdog, which may leave only that inert watchdog group. At or after `READY`, the reaper covers outer hard death with a hung watchdog. Every uncertainty produces fixed quarantine with no retry, replacement, unlink, overwrite, restart or resume.

This coordinator slice is confined to exactly nineteen paths: the prior sixteen-path working boundary plus `src/gate-b-operator-reaper.js`, the inherited-mode addition in `src/gate-b-public-ws-inputs-launcher.js`, and its tests in `test/gate-b-public-ws-inputs.test.js`. The fixture is synthetic, test-only and absent from production imports. It changes no dependency, package script, workflow, runner, wallet, quick-tunnel supervisor or runtime-registration path. Import and local tests invoke no live tunnel, network, RPC, wallet, entropy, funding, signing, payment or publication effect. Owned byte clearing is best-effort and immutable JavaScript strings, including the operator-selected pin, cannot be zeroized. The slice is default-off, unreleased and unactivated, supplies no run authority, and leaves Issue #45 open.
