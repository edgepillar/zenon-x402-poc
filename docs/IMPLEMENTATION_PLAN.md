...
| Local artifact, policy and readiness lane | `MERGED` | `OFFLINE_TESTED` | `RUNTIME_UNREGISTERED` |
| Public-testnet Gate B | `GATE_B_BLOCKED` | `NOT_EXECUTED` | `ISSUE_45_OPEN` |

These status tokens describe repository integration and offline validation only. Merged work is not released, enabled by default, or live-tested. The historical public-testnet profile and operational runner remain behind their exact explicit opt-in gates; only the local-devnet lane is unavailable through ordinary runtime selection. Operator-trusted evidence does not authenticate chain identity, local topology remains unverified, local readiness cannot replace public-testnet Gate B, and no live Gate-B run has occurred.
