...
The journal deliberately stores no mnemonic, private key, seed, token or RPC credential. Cached protected responses are plaintext JSON, are limited to 64 KiB, and may themselves be sensitive; filesystem access to `.runtime/` is therefore part of the PoC trust boundary. Its default capacity is 256 records, and it fails closed at capacity instead of evicting unresolved evidence.
