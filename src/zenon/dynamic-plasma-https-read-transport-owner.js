...
// Default-off, pre-pinned, read-only owner. The existing collector enforces
// UNSIGNED before this adapter receives a request. Neither this transport nor
// its response is durable signing authorization. There is no route discovery.
...
function fixedError(kind) {
  ...
  const message = `Dynamic Plasma HTTPS read transport owner ${suffix}`;
  ...
  put(error, 'code', `dynamic_plasma_https_read_transport_owner_${kind}`);
  return freeze(error);
}
function pending() {
  let resolve;
...
function rejected(kind) {
  ...
  result.reject(fixedError(kind));
  return result.promise;
}
function exact(value, fields) {
  if (value === null || typeof value !== 'object' || isProxy(value) || prototype(value) !== objectPrototype
...
export function createDynamicPlasmaHttpsReadTransportOwner(options) {
  ...
  } catch { throw fixedError('configuration_rejected'); }

  const unavailable = rejected('unavailable');
  const invalidClose = rejected('configuration_rejected');
  ...
  function finishClose() {
    ...
    closeSettled = true;
    if (closeUncertain || !invariant()) reservedClose.reject(fixedError('close_uncertain'));
  ...
  function rejectGeneration(gen) {
    ...
    gen.settled = true;
    gen.outward.reject(fixedError('unavailable'));
  }
  function fail(gen) {
      ...
      const lookup = freeze(function lookup(host, options, callback) {
        ...
        try {
          ...
          if (!called && typeof callback === 'function' && !isProxy(callback)) {
            try { apply(callback, undefined, [fixedError('unavailable')]); } catch { /* no diagnostic inspection */ }
          }
        }
      });
  ...
  const transport = createDynamicPlasmaJsonRpcReadTransport(freeze({ exchange }));
  assertInvariant();
  return freeze({ transport, close });
}
