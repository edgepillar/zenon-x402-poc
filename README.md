...
The source-only `gate-b-operator-front-end` supplies the fixed human entrypoint for the same-process controller workflow. Its direct command uses fixed `/usr/bin/env -i`, the absolute current runtime and that audited module, with no arguments and an empty environment, no shell, `PATH` lookup, package script, run API or activation path. It first validates the trusted controlling TTY without starting a child. The outer launcher then directly starts a fixed detached watchdog and a fixed detached reaper as siblings, retaining both returned handles and process-group identities. The watchdog is inert: before its dedicated private fieldless `START` frame it starts no descendant, reads no bootstrap or review byte, opens no operational resource and emits no operational milestone. The outer writes only its captured watchdog PID to the reaper on one fixed bounded private target pipe; that value never enters arguments, environment, files, output, status or process IPC. The reaper validates the complete frame and EOF and arms its exclusive outer-liveness boundary before returning payload-free `READY`. Only after that joined `READY` and target-write completion does the outer send `START` and return the frozen setup capability.
...
The outer monitors both captured child handles throughout setup and operation. After reaper `READY`, only the reaper may signal the watchdog group while it remains responsive. On reaper loss or timeout the outer first terminates and waits for the captured reaper group, then probes and, only if necessary, takes over TERM, bounded grace, KILL and ESRCH proof for the captured watchdog group. Payload-free `ABSENT` is accepted only after the reaper has proved the watchdog group absent. Clean closure requires watchdog exit, reaper `ABSENT` and exit, and two independent final absence probes of both captured groups; leader exit or an asserted acknowledgement is insufficient. Timeout, malformed or duplicate protocol, early exit, false `ABSENT`, lifecycle uncertainty, configuration drift or residue produces only quarantine, with no retry, replacement, unlink, overwrite, restart or resume. Before reaper `READY`, a responsive inert watchdog closes when the outer dies, and a live outer can TERM/KILL a hung inert watchdog. The accepted dual-fault boundary is narrower: outer hard death before `READY` together with an already unresponsive inert watchdog may leave only that inert watchdog group. No universal pre-`READY` cleanup claim is made. Importing these modules has no process, filesystem, tunnel, network, wallet, signing, payment or publication effect. This offline-tested slice performed none of those effects, is not released or activated, and leaves Issue #45 open.
