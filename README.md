...
Only after setup completes does the front-end write the phase-one prompt, enable raw input and accept the first paste-only canonical JSON line. The line is the complete bootstrap and has a 30-second deadline. Submission first joins the watchdog's one-shot fieldless `BOOTSTRAP_OPEN`/`BOOTSTRAP_OPENED` private control barrier and only then writes canonical bootstrap bytes on the separate private pipe, so the watchdog does not start a competing input deadline before the front-end clock completes. Only after the fixed review-required status may the second line be entered; it has a 120-second deadline and contains only the three exact operator acknowledgements, with no run name, configuration, endpoint or digest. Review submission equivalently joins one-shot fieldless `REVIEW_OPEN`/`REVIEW_OPENED` barriers at both watchdog and inner coordinator before canonical review bytes are written. Raw mode disables terminal echo only while each bounded line is entered. Editing controls, early or surplus bytes, duplicate lines, duplicate, early or stale barriers, duplicate or stale bootstrap submission, restart and resume are rejected. The front-end never echoes or persists payloads. The coordinator, reviewer, controller-only public-input children, tunnel supervisor and tunnel runtime are non-detached and inherit the watchdog group. Ordinary public-input and quick-tunnel launchers retain their detached behavior.
...
The coordinator retains the one controller capability and tunnel lease across `PREPARE`, review, `AUTHORIZE`, preflight and the final hold. The coordinator-specific controller entrypoint launches the tunnel supervisor non-detached, so that supervisor and its non-detached runtime child remain in the watchdog group; the ordinary quick-tunnel entrypoint remains independently detached and unchanged. The fixed reviewer is also non-detached. The documented `telemetryMode` remains an explicit operator-supplied configuration selector; process-control messages are payload-free fixed types, not a claim that the complete workflow has no exposed configuration selector. A valid preflight emits only `GATE_B_CONTROLLER_PREFLIGHT_VALID_RUN_NOT_AUTHORIZED` and still provides no run authority. The hold ends on payload-free `STOP` or the fixed nine-minute lifetime. Controller stop and wait occur exactly once.
...
After the outer has joined both holder `READY` frames with both target-write and handle-transfer callbacks, any surviving holder preserves fixed-denial port custody across any one- or two-holder loss. Outer hard death with a live but hung reaper may leave that reaper, the guard and the watchdog in protective quarantine indefinitely; releasing the last listener on ambiguous watchdog absence is forbidden. At or after both holder `READY` frames and watchdog `START`, only simultaneous hard loss of the outer, reaper and guard while the watchdog/tunnel group survives is the explicit multi-fault residual. A later RUN integration must inherit or broker this same kernel listener with no close/rebind gap, coordinate and retire every active fixed-denial acceptor, and retain the guard as the 503 fallback on server loss; this slice adds no such handoff or RUN authority. Timeout, malformed or duplicate protocol, early exit, false `ABSENT`, lifecycle uncertainty, configuration drift or residue produces only quarantine, with no retry, replacement, unlink, overwrite, restart or resume. Before holder `READY`, a responsive inert watchdog closes when the outer dies, and a live outer can TERM/KILL a hung inert watchdog. The accepted earlier dual-fault boundary remains outer hard death before joined holder readiness together with an already unresponsive inert watchdog, which may leave only that inert watchdog group. Listener custody is not claimed before the outer has joined both holder `READY` frames with both target-write and handle-transfer callbacks; watchdog `START` was never sent, so no tunnel or operational descendant exists. No universal pre-`READY` cleanup claim is made. Module imports are inert. Offline validation uses only synthetic processes and local loopback; it invokes no live tunnel, external-network endpoint, RPC, wallet, funding, signing, payment, publication or RUN effect. The slice is not released or activated and leaves Issue #45 open.
