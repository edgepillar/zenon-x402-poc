...
At the top-level wire envelope, an absent `extensions` property and an empty plain-object `extensions: {}` are accepted as equivalent. `null`, non-empty maps, malformed containers, and unsafe descriptors remain unsupported. This is empty-container wire compatibility only: local clients and servers do not add or advertise `extensions: {}` by default, negotiate or execute extensions, or bind extension content into signing, settlement, journal, or recovery state.
