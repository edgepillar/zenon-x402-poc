...
The strict local `ResourceInfo` profile requires `url`; `description` and `mimeType` are optional strings. Empty strings are accepted for interoperability with official server defaults. Their exact presence and value, including the distinction between absent and empty metadata, remain bound into the payment intent and retained recovery state. Explicit `null` and additional resource members remain unsupported.
...
At the HTTP transport boundary, a missing `PAYMENT-SIGNATURE` receives the ordinary `402` challenge. A present header that cannot be decoded within the bounded transport profile, or whose submitted V2 payload is definitely malformed in shape, container type, or applicable Zenon primitive encoding, receives a private `400` response with `{"error":"invalid_payment"}` and no payment response headers. Structurally valid but unsupported, mismatched, or rejected payments remain `402` with the current challenge. Uncertain settlement remains the PoC-specific `409` reconciliation lane. This is normative HTTP transport alignment, not complete stable-V2 compatibility or exact parity with the released TypeScript resource-server implementation.
