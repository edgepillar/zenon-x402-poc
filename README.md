...
The import-safe `gate-b-buyer-wallet-cli` is a source-only macOS local provisioning helper. An identified operator may explicitly invoke exactly `create --workspace <absolute-private-workspace>` to create the fixed `buyer-wallet.json` and `buyer-address.json` files in an existing canonical owner-only workspace outside Git. The fixed-output supervisor imports no SDK or secret implementation and passes the workspace only through a bounded private bootstrap pipe to one fixed child with empty arguments and a requested empty environment and with ignored standard streams. The child exclusively reserves both files, verifies their current-user mode, Darwin ACL absence, distinct single-link identities and zero length before loading the lockfile-pinned SDK or requesting randomness, derives only account index zero, fully writes and syncs both files and the directory, and never overwrites an existing entry. Unsupported ACL verification fails closed. ACL inspection is a fixed pathname check bracketed by retained-descriptor identity checks; without `openat`-style ACL inspection, any actor already authorized to mutate that namespace can still race the boundary and is inside the private-workspace trust model. The address file remains mode `0600` because even a disposable address is intentionally private before the disclosure gate. JavaScript strings and SDK internals cannot be reliably zeroized; post-write clearing is best effort only. Any failure after reservation preserves restrictive residue because Node has no portable atomic identity-checked unlink; that residue may already contain a valid mnemonic and must be treated as sensitive, inspected or quarantined by the operator, and never blindly deleted or retried. The helper path intentionally invokes no RPC, funding, signing, payment, publication, or Git operation, but its child is not an OS network sandbox and the dynamically imported dependency retains ordinary process capabilities. Offline tests use deterministic construction rather than a real random wallet.
